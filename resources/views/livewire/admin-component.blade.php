               ...
               <div class="card-body">
                   <div class="table-responsive">
                       <table class="table table-bordered text-center" id="dataTable" width="100%" cellspacing="0">
                           ...
                           <tbody>
                               @forelse (App\Models\User::all() as $admin )
                               @if ($admin->role == 'admin')
                                <tr>
                                    ...
                                    <td class="items-center">
                                        ...
                                        <img src="{{ url('storage/' . $admin->profile_photo_path) }}" class="h-10 w-10 rounded-full" alt="User Image">
                                        @else

                                        @endif
                                    </td>
                                    <td><a class="btn btn-sm btn-success" title="Edit {{$admin->name}}" wire:click='edit({{$admin->id}})'> Edit </a> </td>
                             <td><a class="btn btn-sm btn-danger" title="Delete {{$admin->name}}"  data-toggle="modal" data-target="#exampleModal{{$admin->id}}"> X </a> </td>
                         </tr>
                         <!-- Modal -->
                            <div class="">
                                <div class="modal fade" id="exampleModal{{$admin->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <button type="button" class="btn btn-danger" data-dismiss="modal" wire:click='delete({{$admin->id}})'>Delete</button>
                                        </div>
                                    </div>
                                    </div>
                                </div>
